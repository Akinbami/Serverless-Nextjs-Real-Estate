...
const AdminsTable = props => {
    ...
    const columns = [
        {
          ...
          selector: 'firstname',
          sortable: true,
          cell: row => <p>{row.profile?`${row.profile.firstname} ${row.profile.lastname}`:"None"}</p>
        },
        {
          name: 'Role',
          selector: 'role',
    ...
    
        {
            name: 'Delete Action',
            selector: '_id',
            cell: row =><DeleteAdminModal admin={row} token={token} />
    ...
    const [innerData, setInnerData] = useState(props.admins);
    ...
    const save = item => {
        ...
        if (index > -1) {
          ...
          fetch(APPLICATION_API+ `/admins/${payload._id}`, {
            method: 'PUT',
            headers: {
