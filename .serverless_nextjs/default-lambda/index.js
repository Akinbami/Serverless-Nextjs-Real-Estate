...
async function getLanguageRedirectPath(req, manifest, routesManifest) {
    ...
    const headerCookies = req.headers.cookie
        ? (_b = req.headers.cookie[0]) === null || _b === void 0 ? void 0 : _b.value : undefined;
...
const buildS3RetryStrategy = async () => {
    const { defaultRetryDecider, StandardRetryStrategy } = await Promise.resolve().then(function () { return require('./index-0737fe4f.js'); });
...
const triggerStaticRegeneration = async (options) => {
    ...
    const { SQSClient, SendMessageCommand } = await Promise.resolve().then(function () { return require('./index-855e9789.js'); });
...
const s3StorePage = async (options) => {
    const { S3Client } = await Promise.resolve().then(function () { return require('./S3Client-2ce44c80.js'); });
    ...
    const { PutObjectCommand } = await Promise.resolve().then(function () { return require('./PutObjectCommand-ee4a4ff3.js'); });
    await Promise.all([
...
const handleOriginResponse = async ({ event, manifest, prerenderManifest, routesManifest }) => {
    ...
    request.uri = `${basePath}${request.uri.replace(/(\.html)?$/, manifest.trailingSlash ? "/" : "")}`;
    const route = await routeDefault(request, manifest, prerenderManifest, routesManifest);
    ...
    if (fallbackRoute.isStatic) {
        ...
        const { GetObjectCommand } = await Promise.resolve().then(function () { return require('./GetObjectCommand-4007ef8b.js'); });
